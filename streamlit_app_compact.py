...
# ============================================================
# KERAS 3 LIBRARIES FOR BiLSTM
# ============================================================
# Standalone Keras 3 is used here. TensorFlow remains the backend.
# TextVectorization replaces the old Tokenizer + pad_sequences workflow.
# Sequential is used to build the neural network layer by layer
from keras.models import Sequential
# TextVectorization replaces the old Tokenizer + pad_sequences
    ...
    with st.expander("2️⃣ BiLSTM"):
        st.write(
            ...
            **Type:** Deep Learning
            ...
            1. TextVectorization converts words into integer sequences.
...
def train_bilstm():
    """
    Train a Bidirectional LSTM neural network using Keras 3.

    TextVectorization replaces the older Tokenizer and
    pad_sequences workflow.
    """
    # Start timer so training time can be compared with other models
    start_time = time.time()
    # ========================================================
    # CREATE INTERNAL TRAINING / VALIDATION SPLIT
    # ========================================================
    # We do NOT use the final test set for early stopping.
    # 90% of the original training data -> BiLSTM training
    ...
    # LabelEncoder normally maps:
    # negative -> 0, neutral -> 1, positive -> 2
    encoder = LabelEncoder()
    encoder.fit(y_train)
    y_lstm_train_encoded = encoder.transform(y_lstm_train)
    y_lstm_val_encoded = encoder.transform(y_lstm_val)
    # ========================================================
    # TEXT VECTORIZATION SETTINGS
    # ========================================================
    # Maximum vocabulary size
    max_words = 20000
    # Every sentence will be padded/truncated to 100 tokens
    max_length = 100
    # TextVectorization replaces:
    # - Tokenizer()
    # - texts_to_sequences()
    # - pad_sequences()
    ...
    # ========================================================
    # Validation and test text are not used here to avoid data leakage.
    text_vectorizer.adapt(
        X_lstm_train.astype(str).to_numpy()
    )
    # ========================================================
    # CONVERT TEXT INTO FIXED-LENGTH INTEGER SEQUENCES
    # ========================================================
    X_lstm_train_vectorized = text_vectorizer(
        X_lstm_train.astype(str).to_numpy()
    )
    X_lstm_val_vectorized = text_vectorizer(
        X_lstm_val.astype(str).to_numpy()
    )
    ...
    bilstm_model = Sequential([
        # Convert integer word IDs into learned vectors
        Embedding(
            input_dim=max_words,
            output_dim=128,
            mask_zero=True
        ),
        # Read the sentence in both forward and backward directions
        Bidirectional(
            LSTM(
                64,
        ...
        Dropout(0.5),
        # Three output classes: negative / neutral / positive
        Dense(
            3,
            activation="softmax"
        )
    ...
    history = bilstm_model.fit(
        X_lstm_train_vectorized,
        y_lstm_train_encoded,
        validation_data=(
            X_lstm_val_vectorized,
            y_lstm_val_encoded
        ),
        # Maximum epochs selected in Streamlit.
        # Early stopping can finish before this number.
    ...
    # ========================================================
    prediction_probability = bilstm_model.predict(
        X_test_vectorized,
        verbose=0
    )
    ...
    # Count how many epochs were actually completed
...
if st.button(
    ...
    with st.status(
        ...
        st.write(
            "Step 1: Vectorizing sentences with Keras TextVectorization..."
        )
        st.write(
